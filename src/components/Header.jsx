...
export default function Header({ user, title, subtitle }) {
  ...
  return (
    <header className="flex items-center justify-between px-5 py-3 border-b bg-white/90 dark:bg-slate-950/90 backdrop-blur-sm z-20 shrink-0 safe-area-top">
          ...
          <nav className="hidden lg:flex items-center gap-1 ml-2 flex-wrap">
            {navLinks(user.app_role).map((link) => (
              <NavLink
                ...
                to={link.path}
                className={({ isActive }) =>
                  `h-11 flex items-center px-3 rounded-lg text-xs font-medium transition-colors whitespace-nowrap touch-none ${
                    isActive
      ...
      <div className="flex items-center gap-3">
        ...
        <Button variant="ghost" size="icon" className="h-11 w-11" onClick={() => db.auth.logout("/login")}>
