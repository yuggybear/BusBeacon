...
export default function RoleSelector() {
  ...
  const handleSelect = (role) => {
    ...
    db.auth
      ...
      .then(async () => {
        await checkUserAuth();
        navigate(role === "driver" ? "/driver" : "/parent", { replace: true });
      })
      .catch((e) => {
        console.error(e);
        setSaving(null);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100 dark:from-slate-950 dark:to-slate-900 flex flex-col items-center justify-center p-6 safe-area-top safe-area-bottom">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          {roles.map(({ id, label, desc, icon: Icon }) => (
            <button
              ...
              className="group p-6 rounded-2xl bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 hover:border-blue-400 hover:shadow-lg transition-all text-left disabled:opacity-50"
            >
