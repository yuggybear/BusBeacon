...
export default function RouteImport({ user, selectedBus, onImported }) {
  ...
  return (
    <div className="space-y-3 mb-3 p-3 rounded-lg bg-blue-50 dark:bg-blue-950/50 border border-blue-200 dark:border-blue-800">
      <div className="flex items-center gap-2">
        <FileText className="w-4 h-4 text-blue-600" />
        <p className="text-xs font-medium text-blue-700 dark:text-blue-300">Import Route from Document</p>
      </div>
      ...
      {!extracted && !isProcessing && (
        <>
          ...
          <div className="flex items-center gap-2">
            <label className="flex items-center justify-center gap-2 flex-1 h-10 rounded-lg border-2 border-dashed border-blue-300 dark:border-blue-700 bg-white dark:bg-slate-950 cursor-pointer hover:bg-blue-50 dark:hover:bg-blue-950/50 text-sm text-blue-600 dark:text-blue-400 font-medium">
            ...
            <button
              ...
              className="flex items-center gap-1 h-10 px-3 rounded-lg border border-blue-300 dark:border-blue-700 bg-white dark:bg-slate-950 text-xs text-blue-600 dark:text-blue-400 font-medium hover:bg-blue-50 dark:hover:bg-blue-950/50 whitespace-nowrap"
            >
      ...
      {extracted && (
        <div className="space-y-3">
          ...
          <div className="max-h-60 overflow-y-auto space-y-2">
            {extracted.stops.map((stop, i) => (
              <div key={i} className="p-2 rounded-lg bg-white dark:bg-slate-950 border">
                <div className="flex items-center gap-2">
                ...
                {stop.students?.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-1.5 ml-7">
                    {stop.students.map((s, j) => (
                      <span
                        key={j}
                        className="text-xs px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400"
                      >
