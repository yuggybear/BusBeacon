...
import { ShieldCheck, Check, X, UserCog, Crown, ShieldAlert, ShieldX } from "lucide-react";

import { useToast } from "@/components/ui/use-toast";

function VerificationBadge({ admin }) {
  const status = admin.admin_verification_status;
  const score = admin.admin_verification_score;
  ...
  const config = {
    passed: {
      ...
      label: score != null ? `AI Verified (${score}/100)` : "AI Verified",
    },
    flagged: {
      icon: ShieldAlert,
      className: "bg-amber-50 text-amber-700 border-amber-200",
      label: score != null ? `Flagged (${score}/100)` : "Flagged",
    },
    failed: {
      icon: ShieldX,
      className: "bg-red-50 text-red-700 border-red-200",
      label: score != null ? `Rejected (${score}/100)` : "Rejected",
    },
  };
  ...
  const Icon = c.icon;
  return (
    <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full border text-[10px] font-medium ${c.className}`}>
...
export default function AdminVerificationPanel({ schoolName, currentUserId }) {
  ...
  return (
    <div className="p-4 border-b bg-amber-50/50 dark:bg-amber-950/30">
      ...
      {/* Main admin */}
      {mainAdmin && (
        <div className="mb-3 p-2 rounded-lg bg-blue-50 dark:bg-blue-950/50 border border-blue-200 dark:border-blue-800 flex items-center gap-2">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <VerificationBadge admin={mainAdmin} />
            ...
            {mainAdmin.admin_verification_summary && mainAdmin.admin_verification_status !== "passed" && (
              <p className="text-[11px] text-muted-foreground mt-0.5 line-clamp-2">
                {mainAdmin.admin_verification_summary}
              </p>
            )}
          </div>
        </div>
      ...
      {pending.length > 0 && (
        <div className="space-y-2 mb-3">
          ...
          {pending.map((admin) => (
            <div
              key={admin.id}
              className="flex items-center gap-2 p-2 rounded-lg bg-white dark:bg-slate-950 border border-amber-200 dark:border-amber-800"
            >
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium truncate">
                  {admin.full_name || admin.email}
                </p>
                {admin.admin_verification_status && admin.admin_verification_status !== "pending" && (
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <VerificationBadge admin={admin} />
                  </div>
                )}
              </div>
              <button
      ...
      {verified.length > 0 && (
        <div className="space-y-1.5">
          ...
          {verified.map((admin) => (
            <div
              key={admin.id}
              className="flex items-center gap-2 p-2 rounded-lg bg-white dark:bg-slate-950 border"
            >
