...
import { MapPin, Clock, ListOrdered, Users, RefreshCw } from "lucide-react";
...
export default function ParentDashboard() {
  ...
  const { pullDistance, refreshing } = usePullToRefresh(scrollRef, async () => {
    ...
    try {
      const busData = await db.entities.Bus.get(activeBusId);
      setBus(busData);
  ...
  if (!activeBusId) {
    return <Navigate to="/parent-link" replace />;
  }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-64px)] lg:h-screen">
      ...
      <div ref={scrollRef} className="flex flex-col lg:flex-row flex-1 min-h-0 overflow-y-auto lg:overflow-hidden">
        ...
        {/* Info panel */}
        <div className="lg:w-[320px] lg:border-r border-b lg:border-b-0 lg:overflow-y-auto bg-white dark:bg-slate-950 p-4 space-y-3 shrink-0">
          {/* Student selector */}
          {links.length > 1 && (
            <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="p-3 rounded-xl bg-white dark:bg-slate-950 border">
                ...
                return (
                  <div
                    ...
                    className={`flex items-center gap-2 p-2 rounded-lg text-sm ${
                      isParent ? "bg-teal-50 dark:bg-teal-950/50 border border-teal-200 dark:border-teal-800" : ""
                    }`}
                  >
                    <span
